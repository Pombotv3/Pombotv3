...
# Telegram Payments Provider Token from @BotFather (or Stars/dummy token for test)
...
# ---------------- PLANS DEFINITIONS ----------------
PLANS = [
    {"name": "1 Month VIP",     "amount": 79},
    {"name": "2 Months VIP",    "amount": 149},
    {"name": "3 Months VIP",    "amount": 199},
    {"name": "6 Months VIP",    "amount": 299},
    {"name": "1 Year VIP",      "amount": 399},
    {"name": "Lifetime Access", "amount": 499},
    {"name": "Ultra Mega VIP",  "amount": 799},
]
...
# ---------------- SEND REAL COLORED INVOICE BUTTONS ----------------
async def send_colored_plan_buttons(chat_id: int):
    """
    Sends the plans using Telegram's native invoice button mechanism.
    Telegram renders invoice pay buttons with full-width solid colors.
    """
    for plan in PLANS:
        prices = [LabeledPrice(label=plan["name"], amount=plan["amount"] * 100)]
        await bot.send_invoice(
            chat_id=chat_id,
            title=plan["name"],
            description=f"Access pass for {plan['name']}",
            payload=f"plan_{plan['amount']}_{plan['name'].replace(' ', '')}",
            provider_token=PROVIDER_TOKEN,  # If empty or Stars, Telegram renders as dynamic colored button
            currency="INR",
            prices=prices,
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        # pay=True triggers the Telegram native solid colored button
                        InlineKeyboardButton(
                            text=f"Pay ₹{plan['amount']}",
                            pay=True
                        ),
                        InlineKeyboardButton(
                            text="UPI QR ⚡", 
                            callback_data=f"upi_{plan['amount']}_{plan['name'].replace(' ', '')}"
                        )
                    ]
                ]
            )
...
async def start_handler(message: types.Message):
    ...
    # 1. First Message: Images/Videos with caption "💎 GET PREMIUM"
    if media_1:
        await send_dynamic_media_group(bot, message.chat.id, media_1, caption_1)
    else:
        await message.answer(caption_1)

    # 2. Second Message: Plan Media with caption
    if media_2:
        await send_dynamic_media_group(bot, message.chat.id, media_2, caption_2)

    # 3. Real Colored Buttons directly in chat
    await send_colored_plan_buttons(message.chat.id)
...
# Handle UPI QR generation from button
@bot_router.callback_query(F.data.startswith("upi_"))
