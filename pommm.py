...
# ---------------- GRAPHICS & BUTTON GENERATOR ----------------
class OrderState(StatesGroup):
    waiting_for_screenshot = State()

PLANS_CONFIG = [
    {"amount": 79,  "name": "1 Month VIP",       "color": "#27ae60", "code": "p1"},
    {"amount": 149, "name": "2 Months VIP",      "color": "#e74c3c", "code": "p2"},
    {"amount": 199, "name": "3 Months VIP",      "color": "#2980b9", "code": "p3"},
    {"amount": 299, "name": "6 Months VIP",      "color": "#27ae60", "code": "p4"},
    {"amount": 399, "name": "1 Year VIP",        "color": "#e74c3c", "code": "p5"},
    {"amount": 499, "name": "Lifetime Access",   "color": "#2980b9", "code": "p6"},
    {"amount": 799, "name": "Ultra Mega VIP",    "color": "#27ae60", "code": "p7"},
]
...
def render_colored_plan_image():
    """Draws exact colored button bars with text centered inside each bar on transparent background"""
    width = 750
    btn_h = 70
    gap = 14
    total_h = len(PLANS_CONFIG) * (btn_h + gap) + gap
    ...
    for i, plan in enumerate(PLANS_CONFIG):
        y1 = gap + i * (btn_h + gap)
        y2 = y1 + btn_h
        draw.rounded_rectangle([20, y1, width - 20, y2], radius=14, fill=plan["color"])
        
        label = f"{plan['name']} - ₹{plan['amount']}"
...
def make_plan_click_keyboard():
    """Generates direct selection buttons underneath"""
    rows = []
    for plan in PLANS_CONFIG:
        rows.append([
            InlineKeyboardButton(
                text=f"👉 Buy {plan['name']} (₹{plan['amount']})", 
                callback_data=f"buy_{plan['amount']}_{plan['name']}"
            )
        ])
...
async def start_handler(message: types.Message):
    ...
    # 1. Main reply buttons at bottom
    await message.answer("Welcome to Fire World Bot!", reply_markup=make_bottom_reply_keyboard())

    # 2. First Message (Media + Caption "💎 GET PREMIUM")
    if media_1:
        await send_dynamic_media_group(bot, message.chat.id, media_1, caption_1)
    else:
        await message.answer(caption_1)

    # 3. Second Message (Plan media + exact colored buttons)
    if media_2:
        await send_dynamic_media_group(bot, message.chat.id, media_2, caption=None)

    color_buttons_image = render_colored_plan_image()
    photo_file = BufferedInputFile(color_buttons_image.read(), filename="plans.png")

    await message.answer_photo(
        photo=photo_file,
        caption=f"*{caption_2}*\n\nTap below to select your plan:",
        parse_mode="Markdown",
        reply_markup=make_plan_click_keyboard()
    )

@bot_router.message(F.text == "💎 GET PREMIUM")
async def show_plans_only(message: types.Message):
    async with AsyncSessionLocal() as session:
        res_m2 = await session.get(BotConfig, "plan_media")
        res_c2 = await session.get(BotConfig, "plan_caption")
