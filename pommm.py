...
async def init_db():
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not await session.get(BotConfig, "plan_caption"):
            session.add(BotConfig(key="plan_caption", value="═« PRO PLAN »═"))

        # Default demo videos list
...
async def send_first_media_with_bottom_bar(chat_id: int, media_list: list, caption: str):
    """Sends first message media and attaches bottom reply keyboard without sending any extra text."""
    if not media_list:
    ...
    if len(media_list) == 1:
        ...
        await send_raw_telegram_method(method, {
            "chat_id": chat_id,
            key: item["url"],
            "caption": caption,
            "reply_markup": BOTTOM_REPLY_KEYBOARD
...
async def play_demo_videos_one_by_one(chat_id: int):
    """Plays all admin demo videos sequentially, each having its own GET PREMIUM button."""
    async with AsyncSessionLocal() as session:
        res = await session.get(BotConfig, "demo_videos")
    
    videos = json.loads(res.value) if res else []
    if not videos:
        await bot.send_message(chat_id, "No demo videos configured yet.")
        return

    for vid_url in videos:
        await send_raw_telegram_method("sendVideo", {
            "chat_id": chat_id,
            "video": vid_url,
...
async def start_handler(message: types.Message):
    ...
    # 1. First Message: Images/Videos with caption + bottom persistent bar attached directly
    await send_first_media_with_bottom_bar(message.chat.id, media_1, caption_1)

    # 2. Second Message: "👇 Choose an option:" with 3 styled buttons
    await send_raw_telegram_method("sendMessage", {
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div class="bg-gray-800 p-5 rounded-xl border border-gray-700 space-y-3">
        ...
        <form method="POST" action="/admin/config/update" class="space-y-3">
          ...
          <div>
            <label class="block text-xs uppercase text-gray-400 mb-1">Video URLs / File IDs (JSON Array)</label>
