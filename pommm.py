...
async def build_dynamic_plans_keyboard():
    ...
    rows = []
    for p in plans:
        pid, name, amount, validity, _, _ = p
        rows.append([
            {
...
async def send_plan_presentation(chat_id: int):
    ...
    # Fallback to text message if photo URL is unreachable
    if not res or not res.get("ok"):
        await manager.send_raw("sendMessage", {
            "chat_id": chat_id,
            "text": plan_caption,
...
async def handle_start(message: types.Message):
    ...
    # 3. Third Message: Hand pointing emoji (Dispatched via reliable raw method)
    await manager.send_raw("sendMessage", {
        "chat_id": message.chat.id,
        "text": "👇"
    })
