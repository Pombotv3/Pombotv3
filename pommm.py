...
def make_bottom_reply_keyboard():
    # Telegram Bot API style parameter for reply keyboard buttons
    return {
        "keyboard": [
...
# 7 Solid colored buttons alternating Green, Red, Blue
PLANS_RAW_KEYBOARD = {
    "inline_keyboard": [
...
async def send_raw_telegram_method(method: str, payload: dict):
    """Sends raw payload directly to Telegram Bot API preserving the 'style' attribute"""
...
async def send_plan_with_color_buttons(chat_id: int):
    async with AsyncSessionLocal() as session:
    ...
    # Send photo with native colored inline buttons
...
async def start_handler(message: types.Message):
    ...
    # Send styled persistent bottom buttons (Green & Red)
    await send_raw_telegram_method("sendMessage", {
        "chat_id": message.chat.id,
        "text": "Welcome to Fire World Bot!",
        "reply_markup": make_bottom_reply_keyboard()
    })

    # 1. First Message: Images/Videos + caption
    if media_1:
        await send_dynamic_media_group(bot, message.chat.id, media_1, caption_1)
    else:
        await message.answer(caption_1)

    # 2. Second Message: Image with native green, red, blue buttons attached
    await send_plan_with_color_buttons(message.chat.id)
...
async def process_plan_choice(callback: types.CallbackQuery):
    ...
    # Send GET LINK button with success (green) style
    await send_raw_telegram_method("sendMessage", {
