...
async def get_paginated_orders(limit: int = 50, offset: int = 0):
    db = await get_db_connection()
    try:
...
async def receive_screenshot_proof(message: types.Message, state: FSMContext):
    ...
    # Required confirmation text to user
    await manager.send_raw("sendMessage", {
        "chat_id": message.chat.id,
...
# ==========================================
# ADMIN DEMO UPLOADER & FILE ID GRABBER
# ==========================================
@bot_router.message(StateFilter(None), F.video | F.photo | F.document)
async def admin_media_actions(message: types.Message):
    admin_ids = await get_admin_ids()
    ...
    target_video = message.video or (message.document if is_video else None)

    caption_text = (message.caption or "").strip().lower()
    is_reply_demo = bool(message.reply_to_message and (caption_text == "#demo" or "demo" in caption_text))
    
    # If admin captions video with #demo, store to demo list directly
    if is_video and (caption_text == "#demo" or is_reply_demo or "add demo" in caption_text):
        ...
        await message.reply(
            f"✅ <b>Video added to Demo Videos list!</b>\n\n"
            f"<b>File ID:</b> <code>{file_id}</code>\n"
            f"Total demo videos now: {len(lines)}",
            parse_mode="HTML"
        )
        return

    # Standard Admin File ID Grabber
    if message.video:
    ...
    reply_text = (
        ...
        f"<code>{file_id}</code>\n\n"
        f"<i>💡 Direct Demo Upload:</i>\n"
        f"Send any video with caption <code>#demo</code> to add it instantly to your Demo Videos list!"
    )
...
<body class="text-slate-100 min-h-screen flex">
    ...
    <main class="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto">
        ...
        <div class="p-4 md:p-8 max-w-5xl w-full mx-auto space-y-6">
            ...
            <!-- TAB: CUSTOMER ORDERS (50 LIMIT WITH PREV/NEXT AND CSV EXPORT) -->
                ...
                <div class="glass-card rounded-2xl p-5 space-y-4">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-purple-900/40 pb-3">
                        ...
                        <a href="/admin/orders/export-csv" target="_blank" 
                           class="bg-emerald-600/80 hover:bg-emerald-500 text-white font-tech text-xs px-3.5 py-1.5 rounded-xl uppercase tracking-wider transition flex items-center gap-1.5 self-start sm:self-auto">
                    ...
                    <!-- Orders Pagination Controls (50 per page) -->
                    ...
                    <div class="flex items-center justify-between pt-3 border-t border-purple-900/40 font-mono text-xs text-purple-300">
                        ...
                        <div class="flex gap-2">
                            {% if orders_page > 1 %}
                            <a href="/?tab=tab-orders&order_page={{ orders_page - 1 }}" 
...
async def admin_dashboard(
    ...
    try:
        ...
        # Orders Pagination (50 Limit with Prev/Next)
        order_page = max(1, order_page)
        offset_orders = (order_page - 1) * limit
        all_orders, orders_total = await get_paginated_orders(limit=limit, offset=offset_orders)
