        ...
        async with aiohttp.ClientSession() as s:
            async with s.post(url, json=payload) as resp:
                ...
                if not res_data.get("ok"):
                    logging.warning(f"[Telegram API Response Fail] {method}: {res_data}")
                return res_data
        ...
        async def runner():
            while True:
                try:
                    ...
                    logging.error(f"[BotManager] Polling loop error: {e}")
...
# ==========================================
# KEYBOARDS & SOLID-COLORED BUTTONS
# ==========================================
class PaymentStates(StatesGroup):
    waiting_for_screenshot = State()

# 2-Button Main Bottom Keyboard with solid green and red backgrounds
MAIN_REPLY_KEYBOARD = {
    "keyboard": [
...
# 3-Button Extended Menu with solid green, red, and blue backgrounds
OPTIONS_REPLY_KEYBOARD = {
    "keyboard": [
        [{"text": "💎 GET PREMIUM", "style": "success"}],
        [{"text": "🥵 DEMO", "style": "danger"}],
        [{"text": "✅ HOW TO GET PREMIUM", "style": "primary"}]
    ],
    "resize_keyboard": True
}
...
def make_get_link_keyboard(order_id: int):
    return {
        "inline_keyboard": [
            [{"text": "GET LINK", "callback_data": f"submit_{order_id}"}]
        ]
    }

async def build_dynamic_plans_reply_keyboard():
    """Builds subscription plan buttons using solid colored reply keyboard backgrounds."""
    ...
    rows.append([{"text": "🔙 BACK TO MENU", "style": "primary"}])
    return {
        "keyboard": rows,
        "resize_keyboard": True
    }
...
def parse_direct_links(text_block: str, default_type: str = "photo") -> list[dict]:
    ...
    for line in text_block.splitlines():
        ...
        if not line:
            continue
        m_type = default_type
        if any(line.lower().endswith(ext) for ext in [".mp4", ".mov", ".avi", ".mkv"]):
            m_type = "video"
        items.append({"type": m_type, "url": line})
    return items

# ==========================================
# SAFE BOT DISPATCHER LOGIC
# ==========================================
bot_router = Router()
...
async def send_first_media_with_bottom_bar(chat_id: int, media_list: list, caption: str):
    ...
    if len(media_list) == 1:
        ...
        res = await manager.send_raw(method, {
            "chat_id": chat_id,
            key: item["url"],
            "caption": caption,
            "parse_mode": "HTML",
...
async def send_plan_presentation(chat_id: int):
    # 1. Send hardcoded intermediate banner notice
    await manager.send_raw("sendMessage", {
        "chat_id": chat_id,
        "text": " ══════« CHOOSE PLAN »═════"
    })
    ...
    plans_kbd = await build_dynamic_plans_reply_keyboard()

    res = await manager.send_raw("sendPhoto", {
        "chat_id": chat_id,
        "photo": photo_url,
    ...
    # Fallback to text message if photo URL cannot be reached
    if not res or not res.get("ok"):
        logging.warning(f"sendPhoto failed: {res}. Falling back to text message.")
        await manager.send_raw("sendMessage", {
            "chat_id": chat_id,
            "text": plan_caption,
            "reply_markup": plans_kbd
...
async def play_demo_videos_fast(chat_id: int):
    ...
    for item in demo_items:
        try:
            res = await manager.send_raw("sendVideo", {
                "chat_id": chat_id,
                "video": item["url"],
            ...
            if not res or not res.get("ok"):
                await manager.send_raw("sendMessage", {
                    "chat_id": chat_id,
                    "text": f"📺 <b>Demo Video Preview:</b>\n{item['url']}",
                    "reply_markup": DEMO_ITEM_KEYBOARD
                })
        except Exception as e:
            logging.error(f"[Demo Video Error]: {e}")
...
# ==========================================
# ADMIN FILE ID EXTRACTOR HANDLER
# ==========================================
@bot_router.message(F.video | F.photo, StateFilter("*"))
async def admin_get_file_id(message: types.Message, state: FSMContext):
    ...
    reply_text = (
        ...
        f"<code>{file_id}</code>\n\n"
        f"<i>💡 Direct paste enabled! Paste this file ID directly into your Admin Panel.</i>"
    )
...
async def handle_start(message: types.Message, state: FSMContext):
    ...
    # 1. First Message: Images/Videos with caption + initial bottom bar
    await send_first_media_with_bottom_bar(message.chat.id, media_list, first_caption)

    # 2. Second Message: Choose option with solid-colored green, red, and blue buttons
    await manager.send_raw("sendMessage", {
        "chat_id": message.chat.id,
        "text": "👇 Choose an option:",
        "reply_markup": OPTIONS_REPLY_KEYBOARD
    })
...
@bot_router.message(F.text.contains("GET PREMIUM"), StateFilter("*"))
async def on_text_get_premium(message: types.Message, state: FSMContext):
    await state.clear()
    await send_plan_presentation(message.chat.id)

@bot_router.message(F.text.contains("DEMO"), StateFilter("*"))
async def on_text_demo(message: types.Message, state: FSMContext):
    await state.clear()
    await play_demo_videos_fast(message.chat.id)

@bot_router.message(F.text.contains("HOW TO GET PREMIUM"), StateFilter("*"))
async def on_text_how_to(message: types.Message, state: FSMContext):
    await state.clear()
    await message.answer(
        "📖 <b>How to Get Premium:</b>\n\n"
        "1. Click <b>💎 GET PREMIUM</b>\n"
        "2. Choose your preferred plan from the colored list.\n"
...
@bot_router.message(F.text.contains("BACK TO MENU"), StateFilter("*"))
async def on_back_to_menu(message: types.Message, state: FSMContext):
    await state.clear()
    await manager.send_raw("sendMessage", {
        "chat_id": message.chat.id,
        "text": "🏠 Main Menu:",
...
# Handles clicking solid-colored plan buttons
@bot_router.message(F.text.startswith("🔥"), StateFilter("*"))
async def process_plan_choice_text(message: types.Message, state: FSMContext):
    await state.clear()
    plans = await get_all_plans()
    selected_plan = None
    for p in plans:
        if p[1] in message.text:
            selected_plan = p
            break
    ...
    pid, plan_name, amount, validity, _ = selected_plan
    upi_id = await get_setting("upi_id") or "anmolvlv@ibl"
...
async def receive_screenshot_proof(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    await message.reply("✅ Payment screenshot received. Verification in progress.", reply_markup=MAIN_REPLY_KEYBOARD)
