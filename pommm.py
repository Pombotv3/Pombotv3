...
INITIAL_BOT_TOKEN = os.getenv("BOT_TOKEN", "YOUR_BOT_TOKEN")
    ...
    async with DB_WRITE_LOCK:
        ...
        try:
            ...
            defaults = {
                ...
                "first_caption": "🔥 𝐏𝐫𝐞𝐦𝐢𝐮𝐦 𝐕𝐢𝐝𝐞𝐨 𝐏𝐚𝐜𝐤 - 𝐒𝐢𝐫𝐟 ₹𝟔𝟗\n\n✅ 𝐎𝐧𝐞 𝐓𝐢𝐦𝐞 𝐏𝐚𝐲𝐦𝐞𝐧𝐭 \n✅ 𝐋𝐢𝐟𝐞𝐭𝐢𝐦𝐞 𝐀𝐜𝐜𝐞𝐬𝐬\n✅ 𝐈𝐧𝐬𝐭𝐚𝐧𝐭 𝐃𝐞𝐥𝐢𝐯𝐞𝐫𝐲\n\n💎 𝐖𝐡𝐚𝐭 𝐘𝐨𝐮 𝐆𝐞𝐭:\n• 𝐅𝐮𝐥𝐥 𝐇𝐃 𝐐𝐮𝐚𝐥𝐢𝐭𝐲 𝐕𝐢𝐝𝐞𝐨𝐬  \n• 𝐑𝐞𝐠𝐮𝐥𝐚𝐫 𝐍𝐞𝐰 𝐔𝐩𝐝𝐚𝐭𝐞𝐬  \n• 𝟐𝟒𝐱𝟕 𝐒𝐮𝐩𝐩𝐨𝐫𝐭  \n\n𝐏𝐫𝐢𝐜𝐞: ₹𝟔𝟗 𝐎𝐧𝐥𝐲 💰  \n\n══════ ══════ ══════ ══════ ══════",
                "plan_media": "AgACAgUAAxkBAAPraq9xMa2au2Qm0FHpGzxy4oW1T3oAAvUQaxugLnhVQ8gWa-LJ7NwBAAMCAAN4AAM9BA",
            ...
            default_plans = [
                ("plan_1", "💎 𝗖𝗢𝗟𝗟𝗘𝗚𝗘 𝗟𝗘𝗔𝗞𝗦 𝟰𝗞 🔞💦", 79.0, "30 Days", ""),
                ("plan_2", "💎 𝗖𝗛𝗜𝗟𝗗 𝗣𝗢𝗥𝗡 𝟰𝗞 👶🥵", 99.0, "30 Days", ""),
                ("plan_3", "💎 𝗔𝗟𝗟 𝗜𝗡 𝗢𝗡𝗘 𝗣𝗔𝗖𝗞 𝟰𝗞 🤩🤤", 129.0, "30 Days", ""),
                ("plan_4", "💎 𝗩𝗜𝗥𝗔𝗟 𝗠𝗠𝗦 𝗟𝗘𝗔𝗞𝗦 𝟰𝗞 🎬💋", 69.0, "30 Days", ""),
                ("plan_5", "💎 𝗙𝗢𝗥𝗖𝗘𝗗 𝗥𝗔𝗣𝗘 𝟰𝗞 💀😜", 89.0, "30 Days", ""),
                ("plan_6", "💎 𝗠𝗢𝗧𝗛𝗘𝗥 𝗔𝗡𝗗 𝗦𝗢𝗡 𝟭𝟬𝟴𝟬𝗣 😋✨", 59.0, "30 Days", ""),
                ("plan_7", "💎 𝗣𝗔𝗥𝗠𝗔𝗡𝗘𝗡𝗧 𝗩𝗜𝗣 𝗣𝗟𝗔𝗡 👑🤩", 159.0, "Lifetime", ""),
            ]
...
async def get_paginated_orders(limit: int = 50, offset: int = 0, search: str = ""):
    db = await get_db_connection()
    try:
        search_query = f"%{search.strip().lstrip('@').lstrip('#')}%"
        if search.strip():
            async with db.execute("""
                ...
                WHERE CAST(p.id AS TEXT) LIKE ? 
                   ...
                   OR u.username LIKE ? 
                   OR p.plan_name LIKE ?
            """, (search_query, search_query, search_query, search_query)) as cur:
                total_count = (await cur.fetchone())[0]
            ...
            """, (search_query, search_query, search_query, search_query, limit, offset)) as cur:
                rows = await cur.fetchall()
        else:
...
class AdminBatchDemoStates(StatesGroup):
    uploading_demos = State()
...
def parse_direct_links(text_block: str, default_type: str = "photo") -> list[dict]:
    ...
    for line in text_block.splitlines():
        clean_target = line.strip().replace('"', '').replace("'", "")
        if not clean_target:
            continue
        ...
        if any(lower_line.endswith(ext) for ext in [".mp4", ".mov", ".avi", ".mkv", ".webm"]):
            m_type = "video"
        elif clean_target.startswith("BAACAg"):
            m_type = "video"
        elif clean_target.startswith("AgACAg"):
            m_type = "photo"
...
# ==========================================
# UNIVERSAL MEDIA SENDER (CRASH-PROOF FALLBACK)
# ==========================================
...
# ==========================================
@bot_router.message(PaymentStates.waiting_for_screenshot, F.photo | F.document)
async def receive_screenshot_proof(message: types.Message, state: FSMContext):
    ...
    if message.photo:
        ...
        await message.reply("⚠️ Please upload an image screenshot.")
        return
    ...
    async with DB_WRITE_LOCK:
        ...
        try:
            ...
            logging.error(f"[DB Error in Payment Receipt]: {e}")
        finally:
            await db.close()

    await state.clear()
    
    # Direct confirmation text to user
    await manager.send_raw("sendMessage", {
        "chat_id": message.chat.id,
        "text": "✅ Payment screenshot received wait until your payment is checked.",
        "reply_markup": BOTTOM_REPLY_KEYBOARD
    })

    # Dispatch to admin bot
    ...
    if admin_ids and row:
        ...
        reply_markup = builder.as_markup().model_dump(exclude_none=True)
        for aid in admin_ids:
            try:
                ...
                logging.error(f"[Admin Notify Error] Chat {aid}: {e}")

# ==========================================
# ADMIN MULTI-DEMO BATCH UPLOADER FLOW
# ==========================================
@bot_router.message(Command("adddemos"), StateFilter("*"))
async def start_batch_demo_upload(message: types.Message, state: FSMContext):
    admin_ids = await get_admin_ids()
    if message.from_user.id not in admin_ids:
        return

    await state.set_state(AdminBatchDemoStates.uploading_demos)
    await state.update_data(added_count=0)

    builder = InlineKeyboardBuilder()
    builder.button(text="⏹ Finish & Save Demos", callback_data="finish_batch_demos")
    builder.adjust(1)

    await message.reply(
        "🎬 <b>Batch Demo Upload Mode Activated!</b>\n\n"
        "Send or forward <b>one or multiple demo videos</b> right now (albums and single videos supported).\n"
        "Send <code>/done</code> or click below when you are finished.",
        parse_mode="HTML",
        reply_markup=builder.as_markup()
    )

@bot_router.message(AdminBatchDemoStates.uploading_demos, Command("done"))
@bot_router.callback_query(AdminBatchDemoStates.uploading_demos, F.data == "finish_batch_demos")
async def finish_batch_demo_upload(event: types.Message | types.CallbackQuery, state: FSMContext):
    data = await state.get_data()
    added_count = data.get("added_count", 0)
    await state.clear()

    raw_demos = await get_setting("demo_videos")
    total_demos = len([l for l in raw_demos.splitlines() if l.strip()])

    reply_msg = (
        f"✅ <b>Demo Upload Mode Finished!</b>\n\n"
        f"• Newly added videos: <b>{added_count}</b>\n"
        f"• Total active demo videos: <b>{total_demos}</b>"
    )

    if isinstance(event, types.CallbackQuery):
        await event.answer()
        await event.message.answer(reply_msg, parse_mode="HTML")
    else:
        await event.reply(reply_msg, parse_mode="HTML")

@bot_router.message(AdminBatchDemoStates.uploading_demos, F.video | F.document)
async def process_batch_demo_video(message: types.Message, state: FSMContext):
    ...
    raw_demos = await get_setting("demo_videos")
    lines = [l.strip() for l in raw_demos.splitlines() if l.strip()]
    lines.append(file_id)
    await update_setting("demo_videos", "\n".join(lines))

    data = await state.get_data()
    added_count = data.get("added_count", 0) + 1
    await state.update_data(added_count=added_count)

    await message.reply(
        f"🎬 <b>Added!</b> Video #{added_count} captured.\n"
        f"Send more videos or send <code>/done</code> when finished.",
        parse_mode="HTML"
    )

# ==========================================
# ADMIN FILE ID EXTRACTOR (RESTRICTED TO STATE NONE)
# ==========================================
@bot_router.message(StateFilter(None), F.video | F.photo | F.document)
async def admin_media_grabber(message: types.Message):
    admin_ids = await get_admin_ids()
    if message.from_user.id not in admin_ids:
        return
    ...
    reply_text = (
        ...
        f"<code>{file_id}</code>\n\n"
        f"<i>💡 Batch Upload Tip:</i>\n"
        f"Send <code>/adddemos</code> to upload multiple demo videos at once!"
    )
    await message.reply(reply_text, parse_mode="HTML")

# ==========================================
# SYNCHRONIZED BOT APPROVAL / REJECTION HANDLER
# ==========================================
            ...
            try:
                ...
                if act == "approved":
                    ...
                    try:
                        ...
                        await callback.message.answer(f"Order #{pid} APPROVED ✅")
                else:
                    await db.execute("UPDATE payments SET status='rejected' WHERE id=?", (int(pid),))
                    await db.commit()
                    try:
                        await manager.bot.send_message(t_uid, "❌ Payment verification failed. Proof was rejected.")
                    except Exception:
                        pass
                    try:
                        await callback.message.edit_caption(caption=callback.message.caption + "\n\nSTATUS: REJECTED ❌")
                    except Exception:
                        await callback.message.answer(f"Order #{pid} REJECTED ❌")
            finally:
                await db.close()
...
<body class="text-slate-100 min-h-screen flex">
    ...
    <main class="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto">
        ...
        <div class="p-4 md:p-8 max-w-5xl w-full mx-auto space-y-6">
            ...
            <!-- TAB: CUSTOMER ORDERS (SEARCHABLE + 50 LIMIT PAGINATED) -->
                ...
                <div class="glass-card rounded-2xl p-5 space-y-4">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-purple-900/40 pb-3">
                        <div>
                            ...
                            <p class="text-[11px] text-purple-400 font-mono">Showing {{ all_orders|length }} of {{ orders_total }} total orders (Page {{ orders_page }} of {{ orders_total_pages }})</p>
                        </div>
                        <div class="flex items-center gap-2 flex-wrap sm:flex-nowrap">
                            <!-- Integrated Orders Search Form -->
                            <form method="GET" action="/" class="flex items-center gap-2">
                                <input type="hidden" name="tab" value="tab-orders">
                                <input type="text" name="order_search" value="{{ order_search }}" placeholder="Search #ID, User, @name..."
                                       class="bg-[#070410] border border-purple-900/60 rounded-xl px-3 py-1.5 text-xs text-white placeholder-purple-400/50 font-mono focus:outline-none focus:border-cyan-400 w-44 sm:w-52">
                                ...
                                {% if order_search %}
                                <a href="/?tab=tab-orders" class="text-rose-400 hover:text-white text-xs font-mono px-1">✕</a>
                                {% endif %}
                            </form>
                            <a href="/admin/orders/export-csv" target="_blank" 
                               class="bg-emerald-600/80 hover:bg-emerald-500 text-white font-tech text-xs px-3.5 py-1.5 rounded-xl uppercase tracking-wider transition flex items-center gap-1.5 shrink-0">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-xs font-mono text-slate-300 border-collapse">
                            ...
                            <tbody class="divide-y divide-purple-900/30">
                                ...
                                {% else %}
                                <tr><td colspan="7" class="py-8 text-center text-purple-400">No orders found.</td></tr>
                                {% endfor %}
                            </tbody>
                        </table>
                    </div>

                    <!-- Orders Pagination Controls (50 per page with Search Query Persistence) -->
                    {% if orders_total_pages > 1 %}
                    <div class="flex items-center justify-between pt-3 border-t border-purple-900/40 font-mono text-xs text-purple-300">
                        ...
                        <div class="flex gap-2">
                            {% if orders_page > 1 %}
                            <a href="/?tab=tab-orders&order_page={{ orders_page - 1 }}&order_search={{ order_search }}" 
                               class="px-3 py-1.5 rounded-lg bg-purple-900/40 hover:bg-cyan-600 text-white font-tech transition">&larr; Prev 50</a>
                            {% endif %}
                            {% if orders_page < orders_total_pages %}
                            <a href="/?tab=tab-orders&order_page={{ orders_page + 1 }}&order_search={{ order_search }}" 
                               class="px-3 py-1.5 rounded-lg bg-purple-900/40 hover:bg-cyan-600 text-white font-tech transition">Next 50 &rarr;</a>
                            {% endif %}
                        </div>
                    </div>
                    {% endif %}
            ...
            <!-- TAB: BROADCAST (SUPPORTS VIDEO AND FILE_ID) -->
            <div id="tab-broadcast" class="tab-content space-y-6 hidden">
                <form method="POST" action="/admin/broadcast/send" class="glass-card rounded-2xl p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="broadcast_message" rows="4" required placeholder="Type your broadcast message..." class="w-full bg-[#070410] border border-purple-900/60 rounded-xl p-3 text-sm text-white focus:outline-none"></textarea>
                    </div>
                    <div>
                        <label class="block text-xs text-purple-300 mb-2 font-mono uppercase">Media URL or Telegram File ID (Optional)</label>
                        <input type="text" name="broadcast_media" placeholder="https://example.com/media.mp4 or BAACAg... / AgACAg..." class="w-full bg-[#070410] border border-purple-900/60 rounded-xl px-4 py-2.5 text-sm text-white font-mono focus:outline-none">
                        <span class="text-[11px] text-gray-500 mt-1 block">Supports Photo / Video direct links and Telegram File IDs.</span>
                    </div>
                    <div>
                        <label class="block text-xs text-purple-300 mb-2 font-mono uppercase">Media Type</label>
                        <select name="broadcast_media_type" class="bg-[#070410] border border-purple-900/60 rounded-xl px-4 py-2 text-xs text-white font-mono focus:outline-none">
...
async def admin_dashboard(
    ...
    try:
        ...
        # Users Pagination
        page = max(1, page)
        offset_users = (page - 1) * limit
        users_list, users_total = await get_paginated_users(limit=limit, offset=offset_users, search=user_search)
        total_pages = max(1, math.ceil(users_total / limit))

        # Orders Search & Pagination (50 Limit with Prev/Next)
        order_page = max(1, order_page)
        offset_orders = (order_page - 1) * limit
        all_orders, orders_total = await get_paginated_orders(limit=limit, offset=offset_orders, search=order_search)
        orders_total_pages = max(1, math.ceil(orders_total / limit))
...
async def export_all_orders_csv(is_auth: bool = Depends(require_admin)):
    db = await get_db_connection()
    try:
        async with db.execute("""
            SELECT p.id, p.user_id, COALESCE(u.username, 'N/A'), p.plan_name, p.amount, p.status, p.created_at, p.screenshot_file_id
        ...
        writer.writerow(["Order ID", "User ID", "Username", "Plan Name", "Amount (INR)", "Status", "Created At", "Screenshot File ID"])
        for r in records:
            writer.writerow(r)
        ...
        return StreamingResponse(
            ...
            media_type="text/csv",
            headers={"Content-Disposition": f"attachment; filename=all_orders_{int(datetime.now().timestamp())}.csv"}
        )
    finally:
        await db.close()
...
# SYNCHRONIZED WEB ADMIN ACCEPT/REJECT ACTION
...
# ==========================================
# BROADCAST DISPATCHER (PHOTO, VIDEO & FILE_ID SUPPORTED)
# ==========================================
@app.post("/admin/broadcast/send")
async def handle_broadcast_send(
    broadcast_message: str = Form(...),
    broadcast_media: str = Form(""),
    broadcast_media_type: str = Form("auto"),
    is_auth: bool = Depends(require_admin),
):
    if not manager.bot:
    ...
    sent = 0
    cleaned_media = broadcast_media.strip().replace('"', '').replace("'", "")
    
    preferred = "photo"
    if broadcast_media_type == "video":
        preferred = "video"
    elif broadcast_media_type == "photo":
        preferred = "photo"
    elif cleaned_media:
        lower = cleaned_media.lower()
        if any(lower.endswith(ext) for ext in [".mp4", ".mov", ".avi", ".mkv", ".webm"]) or cleaned_media.startswith("BAACAg"):
            preferred = "video"
        else:
            preferred = "photo"
