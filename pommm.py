...
async def send_plan_presentation(chat_id: int):
    # 1. Send hardcoded intermediate banner notice via raw runner
    await manager.send_raw("sendMessage", {
        "chat_id": chat_id,
        "text": " ══════« CHOOSE PLAN »═════"
...
async def handle_start(message: types.Message):
    ...
    # 3. Third Message: Hand pointing emoji via manager.send_raw (guaranteed delivery)
    await manager.send_raw("sendMessage", {
        "chat_id": message.chat.id,
        "text": "👇"
    })
