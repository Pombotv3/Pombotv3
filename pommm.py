...
# ==========================================
# DATABASE LAYER (WAL, INDEXES & ASYNC LOCK)
# ==========================================
        ...
        async def runner():
            while True:
                try:
                    ...
                    logging.error(f"[BotManager] Polling loop notice: {e}")
...
async def send_plan_presentation(chat_id: int):
    # Fixed intermediate notice
    await manager.bot.send_message(chat_id, " ══════« CHOOSE PLAN »═════")[span_6](start_span)[span_6](end_span)[span_7](start_span)[span_7](end_span)
...
async def play_demo_videos_fast(chat_id: int):
    ...
    if not videos:
        await manager.bot.send_message(chat_id, "📺 No demo videos available.")
        return

    # Direct pipelined dispatching for zero lag
    for vid in videos:
...
# ==========================================
@bot_router.message(F.video | F.photo)
async def admin_get_file_id(message: types.Message, state: FSMContext):
    ...
    reply_text = (
        ...
        f"<code>{file_id}</code>\n\n"
        f"<i>💡 Tap to copy and paste this into your Admin Panel for instant loading!</i>"
    )
...
async def handle_start(message: types.Message):
    ...
    # 1. First Message: Images/Videos with caption + bottom persistent keyboard attached directly[span_8](start_span)[span_8](end_span)
    await send_first_media_with_bottom_bar(message.chat.id, media_list, first_caption)

    # 2. Second Message: Choose option with 3 colored buttons[span_9](start_span)[span_9](end_span)
    await manager.send_raw("sendMessage", {
    ...
    # 3. Third Message: Hand pointing emoji[span_10](start_span)[span_10](end_span)
...
async def receive_screenshot_proof(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    await message.reply("✅ Payment screenshot received. Verification in progress.")
...
# ==========================================
# TEMPLATES (UPDATED NAGATO CYBERPUNK THEME)
# ==========================================
...
<body class="text-slate-100 min-h-screen flex">
    ...
    <main class="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto">
        ...
        <div class="p-4 md:p-8 max-w-5xl w-full mx-auto space-y-6">
            ...
            <!-- TAB: MANAGE USERS (SEARCHABLE & PAGINATED) -->
            ...
            <form method="POST" action="/admin/save">
                ...
                <!-- TAB: MEDIA & GREETINGS (EDIT INTERMEDIATE TEXT REMOVED) -->
                <div id="tab-media" class="tab-content space-y-5 hidden">
                    <div class="glass-card rounded-2xl p-6 space-y-4">
                        ...
                        <div>
                            <label class="block text-xs text-purple-300 mb-1 font-mono uppercase">First Message Media (JSON Payload)</label>
                            <textarea id="first_media_txt" name="first_media" rows="3" required class="w-full bg-[#070410] border border-purple-900/60 rounded-xl p-3 text-sm text-white font-mono">{{ first_media }}</textarea>
                        </div>
                        <div>
                        ...
                        <div>
                            <label class="block text-xs text-purple-300 mb-1 font-mono uppercase">Demo Videos (JSON Array or File IDs)</label>
                    ...
                    <div class="glass-card rounded-2xl p-5 space-y-3">
                        <div class="flex items-center justify-between">
                            <h4 class="font-tech text-xs font-bold text-fuchsia-300 uppercase tracking-wider">Direct First Message Media Upload</h4>
        ...
        async function uploadFirstMediaFiles() {
            ...
            try {
                ...
                if (res.ok && data.status === 'success') {
                    statusEl.innerText = "✅ Successfully added " + data.count + " item(s) to First Message!";
                    statusEl.className = "text-xs text-emerald-400 font-mono mt-1";
                    document.getElementById('first_media_txt').value = JSON.stringify(data.media_list, null, 2);
                    input.value = "";
                } else {
                ...
                btn.innerText = "Upload Media";
                badge.classList.add('hidden');
                statusEl.innerText = "❌ Upload error. Please try again.";
                statusEl.className = "text-xs text-rose-400 font-mono mt-1";
            }
        }
...
async def upload_first_media(
    ...
    try:
        ...
        for f in files:
            ...
            media_type = "video" if any(f.filename.lower().endswith(ext) for ext in [".mp4", ".mov", ".avi", ".mkv"]) else "photo"
            file_url = f"{base_url}/static/uploads/{clean_name}"
            entry = {"type": media_type, "url": file_url}
            new_items.append(entry)
            curr.append(entry)
            await asyncio.sleep(0.01)

        await update_setting("first_media", json.dumps(curr))
        return JSONResponse({"status": "success", "count": len(new_items), "media_list": curr})
    except Exception as e:
