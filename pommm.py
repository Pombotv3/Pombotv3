...
DB_NAME = os.path.join(DATA_DIR, "pompom.db")
...
class BotManager:
    ...
    async def start(self, token: str):
        if not token or token == "YOUR_BOT_TOKEN_HERE":
            logging.warning("[BotManager] No valid token set.")
            return
        ...
        try:
            ...
            logging.warning(f"[BotManager] Reset warning: {e}")
...
async def send_plan_presentation(chat_id: int):
    ...
    await manager.bot.send_message(chat_id, " ══════« CHOOSE PLAN »═════")
...
async def notify_payment_approved(user_id: int, plan_name: str):
    ...
    if access_link:
        caption += "\nClick below to access your channel:"
    ...
    try:
        ...
        logging.warning(f"Delivery notice failed: {e}")
...
async def handle_start(message: types.Message):
    ...
    # 1. First Message: Images/Videos with caption + bottom bar attached
...
# ==========================================
# FASTAPI APPLICATION & WEB INTERFACE
# ==========================================
@asynccontextmanager
...
<body class="text-slate-100 min-h-screen flex">
    ...
    <script>
        ...
        // Multiple First Message Media Upload
        async function uploadFirstMediaFiles() {
            ...
            if (!input.files || input.files.length === 0) {
                statusEl.innerText = "⚠️ Please select at least one photo or video.";
