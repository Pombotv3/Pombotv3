...
async def init_db():
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not first_media:
            default_media = [{"type": "photo", "url": "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=800"}]
            session.add(BotConfig(key="first_media", value=json.dumps(default_media)))

        plan_media = await session.get(BotConfig, "plan_media")
        if not plan_media:
            default_plan_media = [{"type": "photo", "url": "https://images.unsplash.com/photo-1579546929518-9e396f3cc809?w=800"}]
...
def generate_colored_plan_image():
    """Generates the solid colored visual button stripes matching the screenshot"""
    width = 600
    stripe_height = 55
    gap = 10
    colors = [
        ("#27ae60", "🟢 1 Month VIP - ₹79"),
        ("#e74c3c", "🔴 2 Months VIP - ₹149"),
        ("#2980b9", "🔵 3 Months VIP - ₹199"),
        ("#27ae60", "🟢 6 Months VIP - ₹299"),
        ("#e74c3c", "🔴 1 Year VIP - ₹399"),
        ("#2980b9", "🔵 Lifetime Access - ₹499"),
        ("#27ae60", "🟢 Ultra Mega VIP - ₹799")
    ]
    height = len(colors) * (stripe_height + gap) + gap
    img = Image.new("RGBA", (width, height), (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)

    for i, (hex_color, label) in enumerate(colors):
        y1 = gap + i * (stripe_height + gap)
        y2 = y1 + stripe_height
        # Rounded colored rectangle
        draw.rounded_rectangle([15, y1, width - 15, y2], radius=10, fill=hex_color)
...
def make_7_plans_keyboard():
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="🟢 1 Month - ₹79", callback_data="plan_79_1MonthVIP")],
            [InlineKeyboardButton(text="🔴 2 Months - ₹149", callback_data="plan_149_2MonthsVIP")],
            [InlineKeyboardButton(text="🔵 3 Months - ₹199", callback_data="plan_199_3MonthsVIP")],
            [InlineKeyboardButton(text="🟢 6 Months - ₹299", callback_data="plan_299_6MonthsVIP")],
            [InlineKeyboardButton(text="🔴 1 Year - ₹399", callback_data="plan_399_1YearVIP")],
            [InlineKeyboardButton(text="🔵 Lifetime - ₹499", callback_data="plan_499_LifetimeVIP")],
            [InlineKeyboardButton(text="🟢 Ultra Mega - ₹799", callback_data="plan_799_UltraMegaVIP")]
        ]
    )
...
async def start_handler(message: types.Message):
    ...
    # Send persistent reply bottom keyboard
    await message.answer("Welcome to Fire World Bot!", reply_markup=make_bottom_reply_keyboard())

    # 1. First Message (Unlimited media + Caption "💎 GET PREMIUM")
    if media_1:
        await send_dynamic_media_group(bot, message.chat.id, media_1, caption_1)
    else:
        await message.answer(caption_1)

    # 2. Second Message (Plan Media + Caption)
    if media_2:
        await send_dynamic_media_group(bot, message.chat.id, media_2, caption_2)

    # 3. Colored Image Bar + 7 Inline Buttons
    color_bar_img = generate_colored_plan_image()
    bar_file = BufferedInputFile(color_bar_img.read(), filename="plans.png")
    
    await message.answer_photo(
        photo=bar_file,
        caption="👇 *Choose your plan below:*",
        parse_mode="Markdown",
        reply_markup=make_7_plans_keyboard()
    )

@bot_router.message(F.text == "💎 GET PREMIUM")
async def show_plans_cmd(message: types.Message):
    async with AsyncSessionLocal() as session:
        res_m2 = await session.get(BotConfig, "plan_media")
        res_c2 = await session.get(BotConfig, "plan_caption")
...
<body class="bg-gray-900 text-gray-100 p-6 md:p-10">
  <div class="max-w-6xl mx-auto space-y-10">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <div class="bg-gray-800 p-6 rounded-xl border border-gray-700 space-y-4">
        <h2 class="text-lg font-bold text-purple-400">2️⃣ Second Message (Plan Media)</h2>
        <form method="POST" action="/admin/config/update" class="space-y-3">
