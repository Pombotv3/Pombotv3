...
# ==========================================
# KEYBOARDS WITH EXACT OLD STYLED COLORS
# ==========================================
...
# The 3-Options Menu with Solid Colors (Green, Red, Blue) as in the Old Interface
...
async def build_dynamic_plans_keyboard():
    """Builds the exact original styled colored inline buttons (Green, Red, Blue alternating)."""
    ...
    for i, p in enumerate(plans):
        ...
        name = p[1]
        amount = p[2]
        color = p[5] if len(p) > 5 and p[5] in palette else palette[i % len(palette)]
        # Sanitize name for clean callback token
        safe_name = name.replace(" ", "")
        rows.append([
            {
                "text": f"{name} - ₹{int(amount)}",
                "callback_data": f"buy_{int(amount)}_{safe_name}",
                "style": color
            }
        ])
...
async def send_first_media_with_bottom_bar(chat_id: int, media_list: list, caption: str):
    ...
    # Ensure the bottom menu remains visible
    await manager.send_raw("sendMessage", {
        "chat_id": chat_id,
        "text": "👇",
        "reply_markup": BOTTOM_REPLY_KEYBOARD
...
async def send_plan_presentation(chat_id: int):
    ...
    # Fallback to text message if photo URL is invalid
    if not res or not res.get("ok"):
        await manager.send_raw("sendMessage", {
            "chat_id": chat_id,
            "text": plan_caption,
...
async def handle_start(message: types.Message, state: FSMContext):
    ...
    # 1. First Message: Media with caption + persistent bottom bar
...
# Handles clicking old styled colored plan buttons (e.g. buy_79_1MonthVIP)
@bot_router.callback_query(F.data.startswith("buy_"), StateFilter("*"))
async def process_plan_choice_old(callback: types.CallbackQuery, state: FSMContext):
    await state.clear()
    await callback.answer()
